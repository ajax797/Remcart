...
describe('order dish', async () => {
    ...
    let foodName = 'Yorkshire Lamb Patties'
    let user='ajax'
    let password='123aj456'
    it('login as user', async () => {
      ...
      await UserLoginPage.Userlogin(user,password)
    ...
    it('add a dish to cart', async () => {    
     ...
     it ('purchase the dish', async () => {       
        ...
        let cartPrice=await UserDishesPage.CartPrice_bt.getValue()
        expect(dishPrice==cartPrice).to.be.true
        expect(await UserDishesPage.foodInCart.getText()==foodName)
        expect(await UserDishesPage.CartQuantity.getValue()=='1')
